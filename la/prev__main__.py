...
def evaluate(tree: Tree, env: dict):
    ...
    if len(tree.children) == 1:
        ...
        return executeFunction(evaluate(tree.children[0], env), env, args)
    elif tree.data == "_ambig":
...
def assignVariable(output, value, env):
    if isinstance(output, Token):
        env["to_compile_string"] += "{} {} {}\n".format("MOV", output, str(evaluate(value, env)))
...
env = dict(LaBuiltins.__dict__)
env["to_compile_string"] = ""
run_codeblock(tree.children[0], env)

import la.LVMCompiler as compiler

compiler_obj = compiler.LVMCompiler.from_string(env["to_compile_string"])

with open("la_generated_bytecode.lac", "wb") as f:
    compiler_obj.save_to(f)
